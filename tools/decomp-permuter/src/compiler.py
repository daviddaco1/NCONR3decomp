...
class Compiler:
    ...
    def compile(self, source: str, *, show_errors: bool = False) -> Optional[str]:
        ...
        try:
            ...
            # RNEEDA: compile.sh is a shell script; native Windows CreateProcess
            # can't exec it directly (no shebang support), so run it via `sh`
            # (Git for Windows), which is on PATH in this project's setup.
            argv = [self.compile_cmd, c_name, "-o", o_name]
