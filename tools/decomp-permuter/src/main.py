    ...
    for d in options.directories:
        ...
        for fname in [compile_cmd, target_o, base_c]:
            if not os.path.isfile(fname):
                ...
                sys.exit(1)
        # RNEEDA: skip the exec-bit check on native Windows -- os.stat()'s
        # st_mode there is emulated from the file extension, not real unix
        # permissions, so compile.sh (invoked via `sh`, see src/compiler.py)
        # would always fail this check regardless of chmod.
        if sys.platform != "win32" and not os.stat(compile_cmd).st_mode & 0o100:
